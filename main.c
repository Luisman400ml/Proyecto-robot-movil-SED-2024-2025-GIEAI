...
uint8_t acciones[4]={GO,BACK,RIGHTGO,STOP}; //Array que en un futuro contendra las consignas de acción a recorrer via UART
...
void ADC_IRQHandler(void) //Cargamos el valor actualizado en las variables respectivas
{
	if(LPC_ADC->ADDR1 & (1<<31)){		// se ha producido una lectura de la tensión del potenciómetro (AD0.1)
		...
		/*
		modo = Burst;						// OBSERVACIÓN: funcionamiento en modo Burst del ADC para obtener los valores actualizados de Batería 
