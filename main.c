...
/////Configuración de pines como GPIOS (Estan preparados dos pines gpios/motor, por si usamos el otro modo. Pero de momento modo driver: gpio+pwm y enable siempre fijo)
 void config_gpios(void){
	 ...
	 if(accion==GO){
		 ciclo_motor1=100-ciclo;
		 ciclo_motor2=100-ciclo;
	 }
	 else if(accion==LEFT){
	 	 ciclo_motor1=100-ciclo;
		 ciclo_motor2=ciclo;
	...
	//Este codigo es el que hay subido en BB para configurar el PWM, solo he cambiado el pin (no se porque lo cambie; pero da igual)
{
  ...
  LPC_PINCON->PINSEL3 |= (1<<5);   // 1.18 PWM1.1 output ///para Motor 1 (De momento da igual, misma velocidad de giro)  (CAMBIAR: no se puede usar)
	LPC_PINCON->PINSEL3 |= (1<<21);   // 1.26 PWM1.6 output ///para Motor 1 (De momento da igual, misma velocidad de giro)
	
	//USAR LOS PINES P1.1 Y P1.3 PARA LOS PWM (DICHO POR EL PROFESOR)
	//PINES P0.0 Y PINES P0.1 PARA LAS DIRECCIONES
...
// HACR SET DUTY LEFT Y SET DUTY RIGHT (POR SEPADARDO CADA UNA)


void config_botones(void)  //VAMOS PREPARANDO BOTONES
{
	
	LPC_PINCON->PINSEL4 |= (0x15 << 20);	//P2.12:10 como EINT2:0 (0b01.0101)
	LPC_SC->EXTMODE |= 7;					//0b111: Activas por flanco...
	LPC_SC->EXTPOLAR &= ~0x7;				//0b000: ...de bajada
...
{
	//Cambio de modo: STOP -> GO -> RIGHT -> LEFT -> BACK -> STOP...
	
	LPC_SC->EXTINT = (1 << 2); //Borramos flag de interrupción
  accion++;	//Operador ternario para conmutar
	if(accion>4){
		accion=1;
	}
	mot_1=accion;
